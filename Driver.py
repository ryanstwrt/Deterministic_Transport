    ...
    for angle in reversed(range(len(mu_n[0]))):
        if angle > 4:
            for cell_num, cell in enumerate(material_cell):
                ...
                if cell_num == 0:
                    angular_flux_pos_rhs[(angle, nrg)] = angular_flux_pos_lhs[(9 - angle, nrg)] * np.exp(-tau) \
                                + (Q[cell_num] / material_data.ix[total, cell]) * (1 - np.exp(-tau))

                    cell_average_angular_flux = (cell_width * Q[cell_num] - mu_n[(0, angle)]
                                * (angular_flux_pos_rhs[(angle, nrg)] - angular_flux_pos_lhs[(9 - angle, nrg)])) \
                ...
                cell_edge_flux_new[(cell_num, nrg)] += angular_flux_pos_rhs[(angle, nrg)] * mu_n[(1, angle)]
                ...
                cell_edge_current_new[(cell_num, nrg)] += angular_flux_pos_rhs[(angle, nrg)] * mu_n[(1, angle)] * mu_n[(0, angle)]
        ...
        # for mu < 0, we remove the - sign, as the change in delta x would create an additional negative to cancel
            ...
            for cell_num, cell in enumerate(reversed(material_cell)):
                ...
                cell_edge_flux_new[(cell_num, nrg)] += angular_flux_pos_lhs[(angle, nrg)] * mu_n[(1, angle)]
                ...
                cell_edge_current_new[(cell_num, nrg)] += angular_flux_pos_lhs[(angle, nrg)] * mu_n[(1, angle)] * mu_n[(0, angle)]
...
cell_edge_flux_new = np.zeros((128, 2))
cell_edge_current_new = np.zeros((128, 2))
cell_edge_flux_old = np.zeros((128, 2))
...
while k_conv < k_convergence or fission_source_conv < fs_convergence:
    ...
    for energy_group in [0, 1]:
        ...
        while 1 < 2:  # source_convergence < source_convergence_test:
            ...
            cell_edge_current_old[:, energy_group] = cell_edge_current_new[:, energy_group]
...
pp.flux_histogram(pin_cell_average, "Pin Averaged Flux with Cell Average Flux", "Pin Cell", "Flux (1/cm^2)", "Fast Flux", "Thermal Flux", scalar_flux_old)
pp.plot_flux(scalar_flux_old, "Flux", "Cell", "Flux (1/cm^2)", "Fast Flux", "Thermal Flux")
pp.plot_flux(cell_edge_flux_old, "Cell Edge Flux", "Cell Edge", "Flux (1/cm^2)", "Fast Flux", "Thermal Flux")
...
pp.plot_flux(cell_edge_current_old, "Cell Edge Current", "Cell Edge", "Flux (1/cm^2)", "Fast Current", "Thermal Current")
